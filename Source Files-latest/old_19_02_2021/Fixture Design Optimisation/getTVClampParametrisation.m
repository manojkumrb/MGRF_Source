% define (T, V) parametrisation 
function [fem, parameter]=getTVClampParametrisation(fem0, fem, inData, parameter, searchDist)
...
if ~isempty(inData.Clamp.Variable.Ps)
    ...
    for i=1:np
        ...
        if inData.Clamp.Variable.Operation(i)~=4 % active
                ...
                % reference points
                Pm0=inData.Clamp.Variable.Ps(i,:);
                ...
                if flagN==false && flagP==false
                    ...
                    % define point into local coordinate frame
                    x=v1*cos(2*pi*t1);
                    y=v1*sin(2*pi*t1);
                    ...
                    % build ref. frame
                    R0l=vector2Rotation(Nm);

                    % transform to global frame
                    Pmp=apply4x4(Pl, R0l, Pm);
                    ...
                    v2=parameter.Clamp.V(2,i);
                    n2=parameter.Clamp.N(2,i);

                    [Ps, flagS]=pointNormal2PointProjection(fem0, Pm0, Nm, idslave);
