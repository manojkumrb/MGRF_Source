...
function fem=femRefresh(fem, ContactMPC)

% fem: fem structure
% ContactMPC: contact pairs to be set on the model
...
if nargin==1 % re-calculate all contact points
    
    disp('---')
    disp('Recalculating unilateral constraints...')
    disp('---')
...
else % use input data set
    
    disp('---')
    disp('Writing previous unilateral constraints...')
    disp('---')
    ...
    fem.Boundary.Constraint.ContactMPC=ContactMPC;
    for i=1:length(ContactMPC)
           fem.Boundary.Constraint.ContactMPC(i).Gap=ContactMPC(i).GapActual; 
           fem.Boundary.Constraint.ContactMPC(i).Value=-ContactMPC(i).GapActual; 
    end
...
if nctpairs>0
    for i=1:nctpairs
       fem.Sol.Gap(i).Gap=ones(1,nnode)*fem.Options.Max; % gap for each contact pair
