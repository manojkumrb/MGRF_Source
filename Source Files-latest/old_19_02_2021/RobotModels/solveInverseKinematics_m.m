...
function varargout=solveInverseKinematics_m(rob)
...
if strcmp(robmodel,'ABB6620') || strcmp(robmodel,'ABB6700-200') || strcmp(robmodel,'ABB6700-235')
    ...
    %--
    qtrack=rob.Kinematics.Theta.Joint.Coordinate(1);
    ...
    qpos=solveInvPosKinematic(rob, qtrack);
    
    % STEP 2: solve inverse rotation problem
    c=1;
    for i=1:4
        qr=solveInvRotKinematic(rob, [qtrack qpos(i,:), 0 0 0]);
...
function q=solveInvPosKinematic(rob, qtrack)

% q: set of fesible solutions [4, 3]
% flag: true/false => solution found / singular position (axis stroke-end) 
...
% solve for q2
q2=solveq2(rob, Pw, qtrack);

% solve for q3
q3(1:2)=solveq3(rob, Pw, [qtrack q2(1) zeros(1,5)]);
q3(3:4)=solveq3(rob, Pw, [qtrack q2(2) zeros(1,5)]);

% solve for q4
q4(1:2)=solveq4(rob, Pw, [qtrack q2(1) zeros(1,5)]);
q4(3:4)=solveq4(rob, Pw, [qtrack q2(2) zeros(1,5)]);
...
% solve inverse position kinematic: solve for "q5", "q6", "q7"
function q=solveInvRotKinematic(rob, qpos)

% qpos=[qtrack q2 q3 q4 0 0 0];
...
% solve q2
function q2=solveq2(rob, Pw, qtrack)

% num. tolerance
eps=rob.Kinematics.Solver.Eps; 

% trasform Pw into frame attached to q2 
Pw=moveToFrame(Pw, rob, [qtrack, zeros(1,6)], 2);
...
%--
% read wrist position into the "0" frame
...
rob.Kinematics.Theta.Joint.Coordinate=q; Ti=solveForwardKinematics(rob);
...
% teta: feasible joint coordinates (nsol, njoint)
% err: solution error [position, rotation](nsol,2)
% flag: true/false
...
for i=1:size(q,1)
    ...
    if csei % now check the location of tool (TCP)
        rob.Kinematics.Theta.Joint.Coordinate=tetac; Ti=solveForwardKinematics(rob);
...
rob.Kinematics.Theta.Joint.Coordinate=teta;
